...
int main(int argc, char **argv) {
    ...
    MPI_File_seek(fh, world_rank * bufsize, MPI_SEEK_SET);
    MPI_File_write(fh, buf, numints, MPI_INT, MPI_STATUS_IGNORE);

    MPI_File_seek(fh, (world_size - 1 - world_rank) * bufsize, MPI_SEEK_SET);
    MPI_File_read(fh, buf, numints, MPI_INT, MPI_STATUS_IGNORE);
...
// Process[0] - calculated average - 0.046875
// Calculated time of execution - 0.001151
